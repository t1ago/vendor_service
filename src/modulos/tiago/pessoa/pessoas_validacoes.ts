...
const REGEX_DOCUMENTO_FEDEREAL_PESSOA_JURIDICA =
  /^\d{2}\.\d{3}\.\d{3}\/\d{4}\-\d{2}$/;
const REGEX_DOCUMENTO_ESTADUAL_PESSOA_FISICA =
...
export const validar_regras_pessoa = (pessoa: any) => {
  ...
  if (pessoa.tipoPessoa == "F") {
    ...
    if (
      REGEX_DOCUMENTO_FEDEREAL_PESSOA_FISICA.test(pessoa.documentoFedereal) ==
      false
    ) {
      return "CPF inválido.";
    }
    ...
    if (
      REGEX_DOCUMENTO_ESTADUAL_PESSOA_FISICA.test(pessoa.documentoEstadual) ==
      false
    ) {
      return "RG inválido.";
    }
    ...
    if (
      REGEX_DOCUMENTO_FEDEREAL_PESSOA_JURIDICA.test(pessoa.documentoFedereal) ==
      false
    ) {
    ...
    if (
      REGEX_DOCUMENTO_ESTADUAL_PESSOA_JURIDICA.test(pessoa.documentoEstadual) ==
      false
    ) {
