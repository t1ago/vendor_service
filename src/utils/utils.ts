...
export const processarRequest = async <T>(
  request: T,
  mapearDados: <D>(data: any) => D,
) => {
  ...
  try {
    ...
    resultado.mensagem = ERROR_MESSAGES.DEFAULT_MESSAGE.replace(
      "{error}",
      erro,
    );
    resultado.data = {};
...
export const autenticadorInterceptador = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
